...
class PredictionService:
    ...
    def analyze_patterns(self, user_id: str, db: Session):
        ...
        # Converter para DataFrame para facilitar envio tabular (ou sumarização se for muito longo)
        data = []
        for log in logs:
            data.append({
        ...
        # Dados de Recuperação (Módulo 8)
        rec_data = []
        for r in recovery_logs:
            rec_data.append({
                "data": r.registrado_em.strftime("%Y-%m-%d"),
        ...
        # Limitar tail para n nã estourar o contexto
        logs_text = df.tail(30).to_string()
        ...
        response = chain.invoke({
            ...
            "std": m_metrics["std"],
            "logs_data": logs_text,
            "rec_data": df_rec.to_string() if not df_rec.empty else "Nenhum dado de recuperação"
        })
